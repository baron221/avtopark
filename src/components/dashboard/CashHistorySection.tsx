            ...
            {confirmedHistory.map((h) => {
              ...
              <div key={h.id} className="flex items-center justify-between gap-2 text-xs">
                <span className="text-muted-2 font-semibold">
                  ...
                  {h.note ? ` · Сабаб: ${h.note}` : ""}
                  {adjusted ? ` · Дастлаб ${formatSom(h.amount)}, буxгалтер тузатди: ${h.confirmedNote}` : ""}
                </span>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className="font-bold text-heading whitespace-nowrap">
                    {formatSom(h.confirmedAmount ?? h.amount)}
                  </span>
                  {revertReceiptAction && (
                    <form
