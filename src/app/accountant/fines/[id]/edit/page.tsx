...
export default async function EditFinePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    <div className="max-w-[480px] mx-auto w-full p-4 sm:p-7">
      ...
      <Card className="p-6 sm:p-8 mt-3">
        <div className="font-heading font-bold text-xl text-heading mb-5">
          Жаримани таҳрирлаш · {fine.user.fullName}
        </div>
        <EditFineForm fineId={fine.id} amount={Number(fine.amount)} reason={fine.reason} />
      </Card>
    </div>
  );
}
