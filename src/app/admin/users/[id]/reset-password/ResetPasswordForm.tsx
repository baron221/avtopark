...
export function ResetPasswordForm({ userId }: { userId: string }) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4">
      ...
      <div>
        <div className="text-[13px] font-extrabold text-body mb-1.5">Yangi parol</div>
        <input
          name="password"
          type="text"
          required
      ...
      {state.success && <p className="text-success text-[13px] font-bold">Parol muvaffaqiyatli yangilandi.</p>}
      <button
        type="submit"
        disabled={pending}
