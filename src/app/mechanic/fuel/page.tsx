  ...
  return (
    <div className="max-w-[1180px] mx-auto w-full p-4 sm:p-7 flex flex-col gap-5">
      ...
      <Card className="overflow-hidden">
        <div className="hidden lg:grid grid-cols-[1.4fr_0.9fr_0.8fr_0.9fr_1fr_1fr] px-6 py-3 bg-page text-xs font-extrabold text-muted-2 uppercase tracking-wide">
          <div>Zapravka</div>
          <div>Shartnoma</div>
          <div>Davr</div>
          <div>Hajm</div>
        ...
        {payments.map((p) => {
          ...
          return (
            <div
              key={p.id}
              className="grid grid-cols-2 lg:grid-cols-[1.4fr_0.9fr_0.8fr_0.9fr_1fr_1fr] gap-y-1 gap-x-2 px-6 py-3.5 border-t border-row-divider items-center text-sm"
            >
        ...
        <div className="flex justify-between px-6 py-3.5 border-t border-row-divider bg-page">
          <span className="font-extrabold text-sm text-heading">Jami</span>
          <span className="font-heading font-extrabold text-sm text-heading">{formatSom(grandTotal)}</span>
        </div>
      </Card>
