import Link from "next/link";
import { notFound, redirect } from "next/navigation";
import { auth } from "@/auth";
import { prisma } from "@/lib/prisma";
import { Card } from "@/components/ui/Card";
import { hasModuleAccess } from "@/lib/access";
import { EditStationPaymentForm } from "./EditStationPaymentForm";

function toISODate(d: Date) {
  return d.toISOString().slice(0, 10);
}

export default async function EditStationPaymentPage({ params }: { params: Promise<{ id: string }> }) {
  const session = await auth();
  if (!session) redirect("/login");
  if (session.user.role !== "MECHANIC" && !(await hasModuleAccess(session.user.role, "FUEL"))) {
    redirect("/coming-soon");
  }

  const { id } = await params;
  const [payment, stations] = await Promise.all([
    prisma.stationPayment.findUnique({ where: { id } }),
    prisma.fuelStation.findMany({ orderBy: { name: "asc" } }),
  ]);
  if (!payment) notFound();

  return (
    <div className="max-w-[480px] mx-auto w-full p-4 sm:p-7">
      <Link
        href="/mechanic/fuel/payments"
        className="inline-flex items-center bg-page border border-border text-muted-2 rounded-lg px-3 py-1.5 text-[13px] font-bold hover:border-primary hover:text-primary hover:bg-primary-tint transition-colors"
      >
        ← Орқага
      </Link>
      <Card className="p-6 sm:p-8 mt-3">
        <div className="font-heading font-bold text-xl text-heading mb-5">Тўловни таҳрирлаш</div>
        <EditStationPaymentForm
          paymentId={payment.id}
          stationId={payment.stationId}
          periodStart={toISODate(payment.periodStart)}
          periodEnd={toISODate(payment.periodEnd)}
          totalVolume={Number(payment.totalVolume)}
          amount={Number(payment.amount)}
          stations={stations.map((s) => ({ id: s.id, name: s.name }))}
        />
      </Card>
    </div>
  );
}
