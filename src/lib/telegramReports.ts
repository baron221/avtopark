...
export async function buildDailySummaryReport(referenceDate: Date = new Date()): Promise<{ message: string }> {
  ...
  // Income-minus-expense for `day` itself, minus only the slice of any
  // same-day owner payout that came out of `day`'s OWN cash — the rest of
  // a payout (whatever it took to clear the balance carried in from the
  // day before) is already accounted for by the "yesterday" line reading 0
  // instead (see getCashLedgerSummary's own referenceDayPayoutFromOwnCash
  // comment), so subtracting the payout's full amount here too would
  // double-count it.
  const dailyBalance = totalIncome - totalExpense - ledger.referenceDayPayoutFromOwnCash;
  ...
  const message =
    ...
    // `day`'s own equation (kirim − расход, plus a same-day owner payout
    // term only when one actually came out of `day`'s own cash — see
    // dailyBalance's own comment; most days have nothing to add here, since
    // a payout that only cleared the carried-in balance is entirely
    // reflected by the "yesterday" line reading 0 instead). "Жами кассадаги
    // пул" is the real running total (kecha + бугун), kept as its own line.
    `${dayMonthLabel(day)} − Қолдиқ: ${formatSom(totalIncome)} − ${formatSom(totalExpense)}` +
    (ledger.referenceDayPayoutFromOwnCash > 0 ? ` − ${formatSom(ledger.referenceDayPayoutFromOwnCash)}` : "") +
    ` = ${signed(dailyBalance)} сум\n\n` +
